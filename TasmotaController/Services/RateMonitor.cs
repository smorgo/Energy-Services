...
using TasmotaController.Models;
using MQTTnet;
...
namespace TasmotaController.Services
{
    ...
    public class RateMonitor
    {
        ...
        private DeviceState _currentState = DeviceState.Unknown;
        public RateMonitor()
        {
        ...
        {
            Console.WriteLine("Tasmota Controller is warming up");
        ...
        public async Task RunAsync()
        {
            try
            {
                ...
                _device = new TasmotaSwitch(factory, _mqttRelayBroker, _relayControlTopic);
                ...
                _mqttRateClient.ApplicationMessageReceivedHandler = new MqttApplicationMessageReceivedHandlerDelegate(async e =>
                {
                    ...
                    if(rateMessage != null)
                    {
                        if(rateMessage.Time > _previousRateTime)
                        {
                            ...
                            {
                                if(_currentState != DeviceState.On || now > _previousStateChange + _minimumStateInterval)
                                {
                                    Console.WriteLine("Turn switch ON");
                                    await _device.TurnOnAsync();
                                    _previousStateChange = now;
                                    _currentState = DeviceState.On;
                                }
                            }
                            else if(rateMessage.Rate > offThreshold)
                            {
                                if(_currentState != DeviceState.Off || now > _previousStateChange + _minimumStateInterval)
                                {
                                    Console.WriteLine("Turn switch OFF");
                                    await _device.TurnOffAsync();
                                    _previousStateChange = now;
                                    _currentState = DeviceState.Off;
                                }
                            }
